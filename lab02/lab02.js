...
const gaussTask = () => {
  ...
  const testB = [7, 2, 1];
  const { solution, detA, invA, condA } = solveGauss(genMatrix(n), genVector(n));
...
const jacobiTask = () => {
  ...
  printMatrix(genMatrix(4, 2));
  console.log("");
  const { solution, iter_fact, iter_theor } = solveJacobi(genMatrix(n, 2), genVector(n), genXFirst(n), 0.001);
  printVector(solution, false);
  console.log("");
